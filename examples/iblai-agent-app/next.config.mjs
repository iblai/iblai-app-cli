...
const nextConfig = {
  ...
  webpack: (config) => {
    ...
    // If both the app and the SDK bundle their own copy of @reduxjs/toolkit
    // or react-redux, RTK Query hooks inside SDK components (e.g. Profile)
    // end up using a different ReactReduxContext than our StoreProvider —
    // so they silently return undefined with no HTTP requests ever fired.
