# build-msix.ps1
# Builds the Tauri app and packages it as MSIX for Microsoft Store / sideloading.
#
# Prerequisites:
#   - Windows 10 SDK (for makeappx.exe and signtool.exe)
#   - Rust toolchain with appropriate target
#   - @tauri-apps/cli installed via npm/pnpm
#
# Usage:
...
if (-not $SkipTauriBuild) {
    ...
    try {
        pnpm exec tauri build --target $RustTarget
    } finally {
        Pop-Location
    }
} else {
...
if (-not $ExePath) {
    Write-Error "Built executable not found. Searched:`n  $($SearchPaths -join "`n  ")`nRun 'pnpm exec tauri build --target $RustTarget' first."
    exit 1
}
...
# Copy AppxManifest and patch for the target
...
# Auto-patch Publisher to match the signing certificate's subject.
# This prevents SignerSign() failures from publisher/cert mismatch.
if ($CertThumbprint) {
    $signingCert = Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.Thumbprint -eq $CertThumbprint }
    if ($signingCert) {
        $manifestContent = $manifestContent -replace 'Publisher="[^"]*"', "Publisher=`"$($signingCert.Subject)`""
        Write-Host "Patched Publisher to match certificate: $($signingCert.Subject)" -ForegroundColor Gray
    } else {
        Write-Warning "Certificate with thumbprint $CertThumbprint not found in Cert:\CurrentUser\My"
    }
}
...
foreach ($icon in $RequiredIcons) {
    ...
    if (Test-Path $src) {
        ...
        Write-Warning "Missing icon: $icon (generate with: pnpm exec tauri icon path/to/icon.png)"
    }
}
...
if ($CertThumbprint) {
    ...
    # Try with RFC 3161 timestamp first (recommended for distribution)
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            $diagCert = Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.Thumbprint -eq $CertThumbprint }
            if ($diagCert) {
                Write-Host "  Cert found:  $($diagCert.Subject)" -ForegroundColor Gray
                Write-Host "  Expires:     $($diagCert.NotAfter)" -ForegroundColor Gray
            } else {
                Write-Host "  Cert NOT FOUND in Cert:\CurrentUser\My" -ForegroundColor Red
                Write-Host "  Run: pnpm tauri:setup:cert" -ForegroundColor White
            }
            [xml]$diagManifest = Get-Content $ManifestDest
            Write-Host "  Manifest Publisher: $($diagManifest.Package.Identity.Publisher)" -ForegroundColor Gray
            exit 1
        }
    }
    ...
    Write-Host "  Unsigned install options:" -ForegroundColor Gray
