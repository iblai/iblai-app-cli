...
class AgentAppGenerator(BaseGenerator):
    ...
    def generate(self) -> None:
        ...
        self._generate_components_json()
    ...
    def _generate_components_json(self) -> None:
        """Generate components.json for shadcn CLI compatibility.

        This enables `npx shadcn@latest add @shadcn-space/<block>` to install
        production-ready UI blocks from shadcnspace directly into the project.
        """
        content = self.render_template("agent/components.json.j2")
        self.write_file(self.output_dir / "components.json", content)
    ...
    def _generate_sso_login_complete(self) -> None:
        ...
        self.write_file(
            self.output_dir / "app" / "sso-login-complete" / "page.tsx", content
        )
    ...
    def _generate_platform_layout(self) -> None:
        ...
        self.write_file(
            self.output_dir / "app" / "platform" / "[tenantKey]" / "layout.tsx", content
        )
    ...
    def _generate_platform_page(self) -> None:
        ...
        self.write_file(
            self.output_dir / "app" / "platform" / "[tenantKey]" / "page.tsx", content
        )

    def _generate_agent_layout(self) -> None:
        """Generate app/platform/[tenantKey]/[agentId]/layout.tsx."""
        content = self.render_template(
            "agent/app/platform/[tenantKey]/[agentId]/layout.tsx.j2"
        )
        self.write_file(
            self.output_dir
            / "app"
    ...
    def _generate_agent_page(self) -> None:
        ...
        content = self.render_template(
            "agent/app/platform/[tenantKey]/[agentId]/page.tsx.j2"
        )
        self.write_file(
            self.output_dir
            / "app"
