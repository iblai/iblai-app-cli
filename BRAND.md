...
## Using with shadcnspace Blocks

When adding shadcnspace UI blocks, use the brand values above to maintain
visual consistency. See the `/iblai-add-shadcn-component` skill for:

- How to replace default colors with ibl.ai brand values
- Composing blocks with SDK components (ChatWidget, ProfileDropdown, NotificationBell)
- Recommended block categories for ibl.ai apps
